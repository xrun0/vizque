...
class StartPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.subtitle = QLabel(
            "Genç KOMEK tarafından hazırlanan VISQUE uygulamasına hoş geldiniz.\n\n"
            "Bir Kategori Seçmek için Aşağıda Verilen Numaralı Kareye Zıplayınız"
        )
        ...
        # Görsel Yolları
        assets = {
            "religion": str(IMG_DIR / "1.png"),
            "english": str(IMG_DIR / "2.png"),
            "math": str(IMG_DIR / "3.png"),
            "random": str(IMG_DIR / "4.png"),
        }

        card1 = self._make_card(
            image_path=assets["religion"],
            title="Din ve Değerler Eğitimi",
            number=1
        )
        card2 = self._make_card(
            image_path=assets["english"],
            title="İngilizce",
            number=2
        )
        card3 = self._make_card(
            image_path=assets["math"],
            title="Matematik",
            number=3
        )
        card4 = self._make_card(
            image_path=assets["random"],
            title="Rastgele Soru",
            number=4
        )
        self._make_clickable(card1, "religion")
        self._make_clickable(card2, "english")
        self._make_clickable(card3, "math")
        self._make_clickable(card4, "random")
    ...
    def _make_card(self, image_path: str, title: str, number: int) -> QWidget:
        card = QWidget()
